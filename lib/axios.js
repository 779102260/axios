 ...
 * 逻辑：
 * 
 * 1. 用户配置 cancelToken ，值需要手动调用由CancelToken.source()产生：
 * {
    token: token, // {reason（已取消的原因），promise（相当于监听事件，待改变状态的promise，调用cancel可以置为resolve，xhr用此方法调用abort方法取消请求）}
    cancel: cancel // 调用此方法取消请求
   }
 * 
 * 2 xhr 检测是否配置 cancelToken，有则通过cancelToken.token.promise监听用户是否取消请求
 * 
 * 3. 用户取消请求
 * 调用 cancelToken.cancel(cacel reason) ，将 reason 填写到 cancelToken.token.reason，然后改变promise状态，触发2步骤的监听
 * 
 * 4. 另外在分发请求时，检测用户是否已经取消请求，如果已经取消请求，则会抛出异常{reason, __CANCEL__: true}，中断后续操作
 * 
 * 5. 在axios.catch可以捕获取消异常，通过axios.isCancel()可以判断（__CANCEL__）出是否时取消抛出的异常
 */
